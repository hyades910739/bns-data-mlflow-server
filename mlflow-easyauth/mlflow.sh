...
echo "Write backend to ${DATABASE_URL}"
echo "Write artifact to ${ARTIFACT_URL}"

exec mlflow server --backend-store-uri $DATABASE_URL --serve-artifacts --artifacts-destination $ARTIFACT_URL --default-artifact-root $ARTIFACT_URL
